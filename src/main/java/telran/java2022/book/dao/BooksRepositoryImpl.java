...
public class BooksRepositoryImpl implements BookRepository {
	...
	public Stream<Book> findByAuthorsName(String authorName) {
		...
		return em.createQuery("select distinct Book from Book b join Book.authors a where a.name = ?1 \n", Book.class)
	...
	public Stream<Book> findByPublisherPublisherName(String publisherName) {
		return em.createQuery("select distinct Book from Book b join b.publisher p where p.publisherName = ?1", Book.class)
